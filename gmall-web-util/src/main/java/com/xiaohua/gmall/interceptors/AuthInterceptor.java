package com.xiaohua.gmall.interceptors;
...
public class AuthInterceptor extends HandlerInterceptorAdapter {
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        String oldToken = CookieUtil.getCookieValue(request, "gmallOldToken", true);
...
//            }
            ...
            String successJson  = HttpclientUtil.doGet("http://localhost:8085/verify?token="+token+"&currentIp="+currentIp);
        ...
        if (loginSuccess) {
            ...
            if(StringUtils.isNotBlank(token)){
                CookieUtil.setCookie(request,response,"gmallOldToken",token,60*60*2,true);
            }
        } else {
            // 没有登录也能用，但是必须验证
            if (success.equals("success")) {
